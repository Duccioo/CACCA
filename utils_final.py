...
def load_data(path, seq_length):
    """
    Legge un file .smi e restituisce:
      • smiles_input  : (N, seq_length)  interi – sequenza di input  ('X' + SMILES + pad)
      • smiles_output : (N, seq_length)  interi – sequenza target (SMILES + 'E' + pad)
      • chars         : tuple           – lista dei caratteri nel vocabolario
      • vocab         : dict {char: id}
      • prop          : (N, num_prop)   – proprietà di condizionamento (float32)
      • length        : (N,)            – lunghezza reale di ogni input (senza pad)
    """
