...
class CVAE(nn.Module):
    def __init__(
        ...
        self.latent_to_decoder_hidden = nn.Linear(
            latent_size, hidden_size * n_rnn_layer
        )
    ...
    def encode(self, x: torch.Tensor, c: torch.Tensor, lengths: torch.Tensor):
        ...
        packed_input = nn.utils.rnn.pack_padded_sequence(
            encoder_input, lengths.cpu(), batch_first=True, enforce_sorted=False
        )
    ...
    def decode(
        self, z: torch.Tensor, x_target: torch.Tensor, lengths: torch.Tensor
    ) -> torch.Tensor:
        """
        ...
        h_0 = hidden_flat.view(
            self.decoder_rnn.num_layers, B, self.decoder_rnn.hidden_size
        )
        ...
        packed_target = nn.utils.rnn.pack_padded_sequence(
            embedded_target, lengths.cpu(), batch_first=True, enforce_sorted=False
        )
        dec_out_packed, _ = self.decoder_rnn(packed_target, (h_0, c_0))
        dec_out, _ = nn.utils.rnn.pad_packed_sequence(
            dec_out_packed, batch_first=True, total_length=T_out
        )
    ...
    def configure_optimizers(
        self, learning_rate, weight_decay=0.01, betas=(0.9, 0.999)
    ):
...
class ConditionalAutoencoder(nn.Module):
    ...
    def encode(
        self, x: torch.Tensor, c: torch.Tensor, lengths: torch.Tensor
    ) -> torch.Tensor:
        B, T_in = x.size()
        ...
        encoder_input = torch.cat(
            [embedded_smiles, c_expanded], dim=2
        )  # -> (B, T_in, embedding_dim + condition_dim)
    ...
    # ---------------------------------------------------------------
    def sample_old(
        self, latent_vector, c, start_token, seq_length=120, greedy=False, device="cpu"
    ):
        """
...
class OLDConditionalAutoencoder(nn.Module):
    def __init__(
        ...
        self.to_latent = nn.Sequential(
            nn.Linear(
                encoder_output_size + self.embedding_prop_output_size, hidden_size
            ),
            nn.GELU(),
            nn.Linear(hidden_size, latent_size),
        )
    ...
    def decode(self, z, x_target, lengths):
        ...
        dec_out, _ = nn.utils.rnn.pad_packed_sequence(
            dec_out_packed, batch_first=True, total_length=T
        )
